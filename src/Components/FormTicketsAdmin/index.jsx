...
export function FormTicketsAdmin({ onSubmit,clients }) {
    ...
    <Card className="w-full max-w-2xl bg-gray-50 dark:bg-gray-800 dark:text-gray-50">
      ...
      <CardContent>
        ...
        <div className="space-y-2">
            <label htmlFor="user" className="text-sm font-medium text-gray-700 dark:text-gray-300">Clientes</label>
            <select
              id="user"
              value={formData.user}
            ...
            >
              ...
              {clients?.map((item)=>(
                <option value={`${item._id}`} key={`${item._id}`}>{item.name} de {item.company.toUpperCase()}</option>
              ))}
